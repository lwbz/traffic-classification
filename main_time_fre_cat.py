...
if __name__ == '__main__':
    ...
    parser.add_argument('--warmup_epochs', type=int, default=5, help='warmup epochs using only labeled data for ssl')
    ...
    for i, train_dataset in enumerate(train_datasets):
        ...
        val_time = torch.from_numpy(val_dataset).float().unsqueeze(1)
        test_time = torch.from_numpy(test_dataset).float().unsqueeze(1)
        ...
        # 拼接时域和频域数据
        train_combined = torch.cat([train_tem, train_freq], dim=1)  # (n_samples, 3, 50)
        ...
        val_combined = torch.cat([val_time, val_freq], dim=1)
        ...
        test_combined = torch.cat([test_time, test_freq], dim=1)

        # 创建数据集
        train_set_labeled = UCRDataset(train_combined.to(device),
                                       torch.from_numpy(train_target).to(device).to(torch.int64))
        val_set = UCRDataset(val_combined.to(device), torch.from_numpy(val_target).to(device).to(torch.int64))
        test_set = UCRDataset(test_combined.to(device), torch.from_numpy(test_target).to(device).to(torch.int64))
        ...
        train_labeled_loader = DataLoader(train_set_labeled, batch_size=batch_size_labeled, num_workers=0,
                                          drop_last=False)
        ...
        # # convert_coeff：
        # #   自定义函数，用于对 FFT 系数进行归一化或其他转换。
        # train_fft, _ = convert_coeff(train_fft)
        #
        # train_set_feq = UCRDataset(train_fft.to(device),
        #                          torch.from_numpy(train_target).to(device).to(torch.int64))
        #
        # # 频域转换：
        # #   使用快速傅里叶变换 (fft.rfft) 提取验证和测试数据的频域特征。
        # #   调用 convert_coeff 对 FFT 系数进行后处理（如归一化或特定变换）。
        # # 构造数据集：
        # #   将频域特征和对应标签封装成 UCRDataset 对象，便于后续加载和训练
        # val_fft = fft.rfft(torch.from_numpy(val_dataset), dim=-1)
        # val_fft, _ = convert_coeff(val_fft)
        ...
        #
        # # 构建频域数据加载器
        # train_loader_feq = DataLoader(train_set_feq, batch_size=args.batch_size, num_workers=0)
        ...
        num_steps = train_set_labeled.__len__() // args.batch_size
        if num_steps == 0:
        ...
        for epoch in range(args.warmup_epochs):
            ...
            val_loss, val_accu_tem, _, _, _ = evaluate(val_loader, model, classifier, loss, device, 0)
            ...
            if min_val_loss > val_loss:
                ...
                end_val_epoch = epoch
                test_loss, test_accuracy_tem, test_recall, test_f1, conf_matrix = evaluate(test_loader, model, classifier, loss, device, 0)
