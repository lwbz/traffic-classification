...
from ts_tfc_ssl.ts_data.preprocessing import normalize_per_series, fill_nan_value, normalize_freq_data
from ts_tfc_ssl.ts_model.loss import sup_contrastive_loss
...
from ts_tfc_ssl.ts_utils import build_model, set_seed, build_dataset, get_all_datasets, \
    ...
    weak_augment, strong_augment, generate_cpl_mask, construct_graph_via_knn_nearind_gpu
...
if __name__ == '__main__':
    ...
    # Dataset setup
    parser.add_argument('--dataset', type=str, default='IOT20',
                        help='dataset(in ucr)')  # IOT21 TMC21 TMC13
    ...
    parser.add_argument('--labeled_ratio', type=float, default=0.03, help='0.1, 0.2, 0.4')
    parser.add_argument('--warmup_epochs', type=int, default=10, help='warmup epochs using only labeled data for ssl')
    ...
    parser.add_argument('--p_cutoff', type=float, default=0.90, help='0.85, 0.9, 0.95')
    parser.add_argument('--confidence_threshold', type=float, default=0.86, help='0.85, 0.9, 0.95')
    ...
    projection_head = ProjectionHead(input_dim=256)
    ...
    for i, train_dataset in enumerate(train_datasets):
        ...
        train_fft, _ = convert_coeff(train_fft, return_type='both')  # (n_samples, 1, 26)
        val_fft = fft.rfft(torch.from_numpy(val_dataset), dim=-1)
        val_fft, _ = convert_coeff(val_fft, return_type='both')
        test_fft = fft.rfft(torch.from_numpy(test_dataset), dim=-1)
        test_fft, _ = convert_coeff(test_fft, return_type='both')

        train_fft = normalize_freq_data(train_fft, mode='both')
        val_fft = normalize_freq_data(val_fft, mode='both')
        test_fft = normalize_freq_data(test_fft, mode='both')

        # 上采样频域数据到长度 50
        train_freq = F.interpolate(train_fft, size=50, mode='linear', align_corners=False)  # (n_samples, 1, 50)
        ...
        train_freq_labeled = train_freq[labeled_indices]
        ...
        train_set = IOTDataset(train_tem.to(device), train_freq.to(device),
                               y_train_all.to(device).to(torch.int64), is_labeled=False)  # 包含所有数据

        labeled_set = IOTDataset(train_tem_labeled.to(device), train_freq_labeled.to(device),
                               torch.from_numpy(train_target_labeled).to(device).to(torch.int64), is_labeled=True)
        ...
        print(f"Labeled samples: {len(labeled_indices)}, Unlabeled samples: {len(unlabeled_indices)}", file=log_file)
        ...
        # 数据加载器
        train_labeled_loader = DataLoader(labeled_set, batch_size=batch_size_labeled, num_workers=0, drop_last=True, shuffle=True)
        ...
        if args.is_sl == 1:
            for epoch in range(1, args.warmup_epochs + 1):
                ...
                if epoch <= args.warmup_epochs:
                    for time_data, freq_data, y, _ in train_labeled_loader:  # train_labeled_loader：仅包含带标签的数据，专用于有监督训练。
                ...
                val_loss, val_accu_tem, _, _, _, _ = evaluate(val_loader, model, classifier, loss, device, 2)
                ...
                print("macro_pre    = {:.3f}, micro_pre    = {:.3f}, weighted_pre    = {:.3f}".format(test_precision[0],
                                                                                             test_precision[1],
                                                                                             test_precision[2]))
                ...
                print("macro_pre    = {:.3f}, micro_pre    = {:.3f}, weighted_pre    = {:.3f}".format(test_precision[0],
                                                                                             test_precision[1],
                                                                                             test_precision[2]), file=log_file)
                print("macro_recall = {:.3f}, micro_recall = {:.3f}, weighted_recall = {:.3f}".format(test_recall[0],
                                                                                                      test_recall[1],
                                                                                                      test_recall[2]),
                      file=log_file)
        ...
        if args.is_pseudo == 1:  #  使用伪标签
            ...
            for epoch in range(1, args.pseudo_epochs + 1):
                ...
                # if epoch > 10 and epoch % 3 == 0 and args.confidence_threshold < 0.92:
                #     args.confidence_threshold += 0.005
                # # confidence_threshold = 0.80 + (0.99 - 0.80) * (epoch - 1) / (args.pseudo_epochs - 1)  # 从 0.80 到 0.99
                # print(f"Pseudo Epoch {epoch}\nconfidence_threshold = {args.confidence_threshold:.4f}")
                print(f"\nPseudo Epoch {epoch}")
                print(f"\nPseudo Epoch {epoch}", file=log_file)
                for time_data, freq_data, y, _ in train_loader:
                    # 获取频域数据和掩码
                    start_idx = num_iterations * args.batch_size
                    end_idx = min((num_iterations + 1) * args.batch_size, len(train_tem))

                    # freq_data_batch = train_freq[start_idx:end_idx].to(device)
                    # y_batch = y_train_all[start_idx:end_idx].to(device)

                    # 直接使用 train_loader 提供的数据
                    freq_data_batch = freq_data.to(device)
                    y_batch = y.to(device)
                    ...
                    # 提取嵌入和预测
                    pred, pred_embed = model(time_data, freq_data_batch)  # 联合预测

                    # print(f"\nEmbed norm mean: {torch.norm(pred_embed, dim=1).mean():.3f}, std: {torch.norm(pred_embed, dim=1).std():.3f}")
                    ...
                    if not queue_train_x.full():
                        ...
                        # 额外置信度筛选（不依赖真实标签）
                        # pseudo_probs = torch.softmax(pred, dim=-1)  # 模型对当前批次的预测概率
                        # max_probs, model_pred_labels = torch.max(pseudo_probs, dim=-1)
                        # pseudo_labels_batch = knn_result_label[-len(y):]
                        # consistency_mask = (model_pred_labels == pseudo_labels_batch).cpu()  # 模型预测与伪标签一致性
                        # avg_confidence = max_probs.mean().item()  # 当前批次平均置信度
                        # consistency_mask_mean = consistency_mask.float().mean()
                        # if avg_confidence < args.confidence_threshold or consistency_mask_mean < args.confidence_threshold:  # 置信度或一致性不足
                        #     mask_cpl_batch[:] = False  # 丢弃低质量伪标签
                        ...
                        # if len(valid_pseudo) > 0:# 高置信度伪标签的总数
                        #     pseudo_accuracy = total_correct_pseudo / total_pseudo
                        # else:
                        #     pseudo_accuracy = 0.0
                        # sys.stdout.write(
                        #         f"\rAvg_confidence: {avg_confidence:.4f}, Consistency_mask_mean: {consistency_mask_mean:.4f}, Pseudo-label accuracy: {pseudo_accuracy:.4f}, Confident labels: {total_pseudo}")
                        # sys.stdout.flush()

                            # # 计算当前批次的伪标签正确率  作弊了
                            # batch_acc = (valid_pseudo == valid_true).sum() / len(valid_pseudo)
                            # if batch_acc < 0.85:  # 只接受正确率 > 0.85 的批次
                            #     mask_cpl_batch[:] = False  # 丢弃低质量伪标签
                    ...
                    # 现状：嵌入范数均值 16.014，std 9.896，可能过分离。
                    # 建议：添加范数正则化，控制嵌入幅度。
                ...
                print(f"\navg_labeled_loss = {avg_labeled_loss:.4f}, avg_pseudo_loss = {avg_pseudo_loss:.4f}", file=log_file)
                ...
                if total_pseudo > 0:
                    ...
                    print(
                            f"\nAverage Pseudo-label accuracy: {avg_pseudo_accuracy:.4f}, Total Confident labels: {total_pseudo}", file=log_file)
                ...
                print("cal_loss_label_num = {} + {} = {}".format(len(labeled_indices), cal_loss_label_num - len(labeled_indices), cal_loss_label_num))
                print("cal_loss_label_num = {} + {} = {}".format(len(labeled_indices), cal_loss_label_num - len(labeled_indices), cal_loss_label_num), file=log_file)
    ...
    # 保存模型
    torch.save(model.state_dict(), os.path.join(log_path, f"model_ratio_{args.labeled_ratio}_acc_{acc}.pth"))
    if is_projection_head:
        torch.save(projection_head.state_dict(),
                   os.path.join(log_path, f"projection_head_ratio_{args.labeled_ratio}_acc_{acc}.pth"))
    # torch.save(classifier.state_dict(), os.path.join(log_path, f"classifier_epoch_{args.pseudo_epochs}.pth"))
    print(f"Model saved to {log_path}/model_ratio_{args.labeled_ratio}_acc_{acc}.pth")

    old_name = log_path
    new_name = log_path + "_" + acc + "_" + pre + "_" + rc + "_" + f1
    if args.is_pseudo == 1:
        new_name = new_name + "_pseudo_knn_only_amp_or_phase"
    else:
        new_name = new_name + "_base_"
    new_name = new_name + str(args.labeled_ratio) + "_" + str(len(sum_target)) + "_main_knn"
