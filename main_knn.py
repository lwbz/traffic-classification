...
if __name__ == '__main__':
    ...
    # Base setup
    parser.add_argument('--backbone', type=str, default='CNNMambaFusionModel', help='ParallelLSTMModel, FCN_Time, FCN_Freq, Time_Freq, Transformer_TF')
    parser.add_argument('--random_seed', type=int, default=42, help='shuffle seed')

    # Dataset setup
    parser.add_argument('--dataset', type=str, default='IOT17',
                        help='dataset(in ucr)')  #IOT17 IOT21 TMC21 TMC13 Medboit USTCTFC
    ...
    print("上采样后的类别分布:", Counter(sum_target))
    ...
    for i, train_dataset in enumerate(train_datasets):
        ...
        # 上采样频域数据到长度 50
        train_freq = F.interpolate(train_fft, size=args.seq_len, mode='linear', align_corners=False)  # (n_samples, 2, 50)
        val_freq = F.interpolate(val_fft, size=args.seq_len, mode='linear', align_corners=False)
        test_freq = F.interpolate(test_fft, size=args.seq_len, mode='linear', align_corners=False)
            ...
            for epoch in range(1, args.pseudo_epochs + 1):
                ...
                for time_data, freq_data, y, _ in train_loader:
                    ...
                    if not queue_train_x.full():
                        ...
                        # end_knn_label, mask_cpl_knn = construct_graph_via_knn_nearind_gpu(
