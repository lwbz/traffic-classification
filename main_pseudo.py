...
if __name__ == '__main__':
    ...
    parser.add_argument('--warmup_epochs', type=int, default=20, help='warmup epochs using only labeled data for ssl')
    ...
    for i, train_dataset in enumerate(train_datasets):
        ...
        # 创建子数据集
        labeled_time = train_tem[labeled_indices]
        labeled_freq = train_freq[labeled_indices]
        labeled_target = train_target[labeled_indices]

        labeled_set = IOTDataset(labeled_time.to(device), labeled_freq.to(device),
                                 torch.from_numpy(labeled_target).to(device).to(torch.int64))

        unlabeled_time = train_tem[unlabeled_indices]
        unlabeled_freq = train_freq[unlabeled_indices]
        unlabeled_set = IOTDataset(unlabeled_time.to(device), unlabeled_freq.to(device))  # 无标签

        print(f"Labeled samples: {len(labeled_set)}, Unlabeled samples: {len(unlabeled_set)}")
        print(f"Labeled samples: {len(labeled_set)}, Unlabeled samples: {len(unlabeled_set)}", file=log_file)
        ...
        while labeled_time.shape[0] < batch_size_labeled:
        ...
        # 数据加载器
        labeled_loader = DataLoader(labeled_set, batch_size=batch_size_labeled, num_workers=0, shuffle=True)
        unlabeled_loader = DataLoader(unlabeled_set, batch_size=args.batch_size, num_workers=0)
        ...
        for epoch in range(1, args.warmup_epochs + 1):
            ...
            if epoch < args.warmup_epochs + 1:
                # for time_data, freq_data, y in train_loader:  # train_labeled_loader：仅包含带标签的数据，专用于有监督训练。
                for time_data, freq_data, y in labeled_loader:
                    # time_data, freq_data, y = time_data.to(device), freq_data.to(device), y.to(device)
                    optimizer.zero_grad()
                ...
                # print(f"Initial Epoch {epoch}, Loss: {step_loss.item()}")
                # print(f"Initial Epoch {epoch}, Loss: {step_loss.item()}", file=log_file)
        ...
        print(f"Pseudo-labeled samples: {len(pseudo_set)}")
        print(f"Pseudo-labeled samples: {len(pseudo_set)}", file=log_file)

        # 混合训练
        combined_set = ConcatDataset([labeled_set, pseudo_set])
        combined_loader = DataLoader(combined_set, batch_size=args.batch_size, num_workers=0, shuffle=True)

        for epoch in range(20):
            model.train()
            for time_data, freq_data, target in combined_loader:
