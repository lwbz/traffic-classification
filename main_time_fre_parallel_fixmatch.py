...
from ts_tfc_ssl.ts_utils import build_model, set_seed, build_dataset, get_all_datasets, \
    ...
    weak_augment, strong_augment, generate_cpl_mask

if __name__ == '__main__':

    parser = argparse.ArgumentParser()
    # Base setup
    parser.add_argument('--backbone', type=str, default='Transformer_TF', help='encoder backbone, FCN_Time, FCN_Freq, Time_Freq, Transformer_TF')
    ...
    # Semi training
    parser.add_argument('--is_sl_only', type=int, default=0, help='0, 1')
    parser.add_argument('--is_pseudo', type=int, default=1, help='0, 1')
    parser.add_argument('--labeled_ratio', type=float, default=0.05, help='0.1, 0.2, 0.4')
    parser.add_argument('--warmup_epochs', type=int, default=100, help='warmup epochs using only labeled data for ssl')
    ...
    for i, train_dataset in enumerate(train_datasets):
        ...
        if args.is_sl_only == 1:
            for epoch in range(1, args.warmup_epochs + 1):
                ...
                if stop_count == 60 or increase_count == 60:
                    print(
                        'model convergent at epoch {0}, early stopping. stop_count = {1}, increase_count = {2}'.format(
        ...
        if args.is_pseudo == 1:  #  使用伪标签
            # 初始化类计数和准确率
            class_counter = Counter(train_target)  # 有标签数据的类别分布
            max_count = max(class_counter.values())
            classwise_acc = torch.zeros(args.num_classes, device=device)
            for i in range(args.num_classes):
                classwise_acc[i] = class_counter.get(i, 0) / max_count  # 初始基于训练集分布

            for epoch in range(1, args.pseudo_epochs + 1):
                model.train()
                unlabeled_weight = min(1.0, epoch / 20.0)

                # p_cutoff = 0.95 * (1 - epoch / args.pseudo_epochs) + 0.7 * (epoch / args.pseudo_epochs)  # 从 0.95 到 0.7
                p_cutoff = 0.90

                cnt = 0
                prop = 0
                for (time_data_l, freq_data_l, target_l, _), (time_data_u, freq_data_u, _, _) in zip(train_loader, unlabeled_loader):

                    optimizer.zero_grad()

                    # 有标签数据
                    output_l = model(time_data_l, freq_data_l)
                    labeled_loss = loss(output_l, target_l)

                    # 未标记数据
                    weak_time, weak_freq = weak_augment(time_data_u, freq_data_u)
                    strong_time, strong_freq = strong_augment(time_data_u, freq_data_u, device)

                    with torch.no_grad():
                        weak_output = model(weak_time, weak_freq)
                        weak_probs = F.softmax(weak_output, dim=1)
                        max_probs, pseudo_labels = torch.max(weak_probs, dim=1)
                        # mask = max_probs >= 0.9
                        mask = generate_cpl_mask(max_probs, pseudo_labels, classwise_acc, p_cutoff=p_cutoff)


                    strong_output = model(strong_time, strong_freq)
                    pseudo_loss = loss(strong_output[mask], pseudo_labels[mask]) if mask.sum() > 0 else 0

                    cnt = mask.sum()
                    prop = max_probs.mean()

                    step_loss = labeled_loss + unlabeled_weight * pseudo_loss

                    step_loss.backward()
                    optimizer.step()

                print("\nmask.sum() = {0}, max_probs.mean() = {1}".format(cnt.item(), prop.item()))
