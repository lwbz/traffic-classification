...
if __name__ == '__main__':
    ...
    # Base setup
    parser.add_argument('--backbone', type=str, default='FCN_Freq', help='encoder backbone, FCN_Time, FCN_Freq, Time_Freq')
    ...
    parser.add_argument('--pseudo_weight', type=float, default=0.30)
    ...
    for i, train_dataset in enumerate(train_datasets):
        ...
        mask_train = np.ones(len(train_freq), dtype=int)
        ...
        # 同步打乱
        indices = np.random.RandomState(args.random_seed).permutation(len(train_freq))  # 随机排列索引
        ...
        if args.is_sl == 1:
            for epoch in range(1, args.warmup_epochs + 1):
                ...
                val_loss, val_accu_tem, _, _, _, _ = evaluate(val_loader, model, classifier, loss, device, 1)
                ...
                test_loss, test_accuracy_tem, test_precision, test_recall, test_f1, conf_matrix = evaluate(test_loader, model, classifier, loss,
                                                                                  device, 1)
        ...
        if args.is_pseudo == 1:
            # 队列
            queue_train_x = queue.Queue(maxsize=args.queue_maxsize)
            queue_train_y = queue.Queue(maxsize=args.queue_maxsize)
            queue_train_mask = queue.Queue(maxsize=args.queue_maxsize)
            ...
            for epoch in range(1, args.pseudo_epochs + 1):
                ...
                for x, y in train_loader:
                    ...
                    end_idx = min((num_iterations + 1) * args.batch_size, len(train_freq))
                    y_batch = y.to(device)
    ...
    new_name = new_name + "_" + str(args.labeled_ratio) + "_" + str(len(sum_target)) + "_main_freq_only"
