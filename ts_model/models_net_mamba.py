...
def net_mamba_pretrain(**kwargs):
    model = NetMamba(
        is_pretrain=True, img_size=IMG_SIZE, stride_size=STRIDE_SIZE, embed_dim=256, depth=4,
...
def net_mamba_classifier(**kwargs):
    model = NetMamba(
        is_pretrain=False, img_size=IMG_SIZE, stride_size=STRIDE_SIZE, embed_dim=256, depth=4,
        **kwargs)
    return model
...
def main():
    ...
    # 直接使用 NetMamba 创建分类器模型
    classifier_model = NetMamba(
        img_size=72,  # 图像大小
        stride_size=2,  # 步长大小
        in_chans=1,  # 输入通道数（灰度图为1）
        embed_dim=512,  # 嵌入维度
        depth=4,  # 深度（Mamba块的数量）
        num_classes=10,  # 分类类别数
        is_pretrain=False,  # 不是预训练模式
        device=device  # 指定设备
    ).to(device)

    # 创建输入张量，形状为 (batch_size, 1, img_size, img_size)
    batch_size = 2
    input_tensor = torch.randn(batch_size, 1, 72, 72).to(device)

    # 前向传播
    output = classifier_model(input_tensor)

    # 打印输出形状
    print(f"Classifier model output shape: {output.shape}")  # 应该为 [batch_size, num_classes]
