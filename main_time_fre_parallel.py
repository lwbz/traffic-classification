...
if __name__ == '__main__':
    ...
    # Base setup
    parser.add_argument('--backbone', type=str, default='CNNMambaFusionModel', help='encoder backbone,LSTMMambaFusionModel, CNNMambaFusionModel, ParallelMambaModel, FCN_Time, FCN_Freq, Time_Freq, Transformer_TF')
    ...
    # Semi training
    parser.add_argument('--labeled_ratio', type=float, default=1.0, help='0.1, 0.2, 0.4')
    ...
    for i, train_dataset in enumerate(train_datasets):
        ...
        # train_labeled_set = IOTDataset(train_tem_labeled.to(device), train_freq_labeled.to(device),
        ...
        #                           drop_last=False, sampler=RandomSampler(train_set, num_samples=int(args.labeled_ratio * len(train_set))))

        # val_loader = DataLoader(val_set, batch_size=args.batch_size, num_workers=0, sampler=RandomSampler(val_set, num_samples=int(args.labeled_ratio * len(val_set))))
        # test_loader = DataLoader(test_set, batch_size=args.batch_size, num_workers=0, sampler=RandomSampler(test_set, num_samples=int(args.labeled_ratio * len(test_set))))

        # train_labeled_loader = DataLoader(train_labeled_set, batch_size=batch_size_labeled, num_workers=0, drop_last=True,
        #                                   shuffle=True)
        ...
        for epoch in range(1, args.warmup_epochs + 1):
            ...
            test_loss, test_accuracy_tem, test_precision, test_recall, test_f1, conf_matrix = evaluate(test_loader,
                                                                                                       model,
                                                                                                       classifier, loss,
                                                                                                       device,
                                                                                                       2)
    ...
    new_name = new_name + "_main_time_fre_parallel"
